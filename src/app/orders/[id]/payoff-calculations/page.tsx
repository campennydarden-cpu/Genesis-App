...
export default async function PayoffCalculationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [payoffs, contacts] = await Promise.all([listPayoffsForCalculation(orderId), listAllContacts(orderId)])
  ...
  return <PayoffCalculationsPanel orderId={orderId} payoffs={payoffs} chargesByPayoff={chargesByPayoff} contacts={contacts} />
}
