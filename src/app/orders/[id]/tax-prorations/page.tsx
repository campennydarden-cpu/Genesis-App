...
export default async function TaxProrationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [prorations, contacts] = await Promise.all([listProrations(orderId), listAllContacts(orderId)])

  return <TaxProrationsPanel orderId={orderId} prorations={prorations} contacts={contacts} />
}
