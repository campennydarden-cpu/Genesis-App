...
test('add policy, edit fields, autosave persists after reload', async ({ page }) => {
  ...
  await card.locator('select[name="bill_code"]').selectOption('PREMIUMS')
  await card.getByLabel('Coverage Amount').fill('300000')
  await card.getByLabel('Base Premium').fill('1200')
  await card.getByLabel('Final Premium').fill('1200')
  await card.getByLabel('Final Premium').blur()
  ...
  await expect(reloadedCard.getByLabel('Coverage Amount')).toHaveValue('$300,000.00')
  await expect(reloadedCard.locator('select[name="bill_code"]')).toHaveValue('PREMIUMS')
})
...
test('add endorsement under a policy with its own split', async ({ page }) => {
  ...
  await card.getByLabel('Charge').fill('75')
  await card.getByLabel('Charge').blur()
  await expect(page.getByText('Saved')).toBeVisible()

  await page.reload()
  const reloadedCard = page.getByTestId('premium-list').locator('[data-testid^="premium-"]').first()
  await expect(reloadedCard.locator('input[name="code"]')).toHaveValue('ALTA 9')
  await expect(reloadedCard.getByLabel('Charge')).toHaveValue('$75.00')
