...
test('add policy, edit fields, autosave persists after reload', async ({ page }) => {
  ...
  await Promise.all([page.waitForNavigation(), card.locator('input[name="bill_code"]').blur()])
...
test('add split under a policy', async ({ page }) => {
  ...
  await Promise.all([page.waitForNavigation(), card.locator('input[name="percent"]').blur()])
...
test('add endorsement under a policy with its own split', async ({ page }) => {
  ...
  await Promise.all([page.waitForNavigation(), card.locator('input[name="charge"]').blur()])
