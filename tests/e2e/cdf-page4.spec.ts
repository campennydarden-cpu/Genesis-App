...
test('nav shows CDF Page 4 under Escrow / Closing group', async ({ page }) => {
  ...
  await expect(group.getByRole('link', { name: 'CDF Page 4' })).toBeVisible()
})

test('fill disclosure checkboxes and escrow radio, autosave persists after reload', async ({ page }) => {
  const orderId = await createOrder(page)
  await page.goto(`/orders/${orderId}/cdf-page-4`)

  await page.locator('#has_demand_feature').check()
  ...
  await page.reload()
  await expect(page.locator('#has_demand_feature')).toBeChecked()
  await expect(page.locator('input[name="escrow_type"][value="Escrow"]')).toBeChecked()
  await expect(page.locator('input[name="escrow_monthly_payment"]')).toHaveValue('350')
})
